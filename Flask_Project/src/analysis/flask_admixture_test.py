...
db_path = "/Users/farzadhamzawe/group_project bioinformatics/Software-Development-Project/instance/ArchGenome.db"
...
def plot_admixture_with_sample_labels(admixture_data, k_value, colors, population_label):
    ...
    ax.set_xticklabels(sample_labels, rotation=45, ha='center')

    # Set the xlabel to '[Population] Population'
    ax.set_xlabel(f'{population_label} Population')

    # Set the y-label and title
    ax.set_ylabel('Ancestry Proportion')
    ax.set_title(f'Admixture Results K={k_value}')  # Updated title

    # Create a legend with colored patches
    patches = [mpatches.Patch(color=colors[i], label=f'Ancestry {i+1}') for i in range(len(colors))]
...
def plot_admixture_for_superpopulation(superpopulation):
    ...
    fig_k3, ax_k3 = plt.subplots(figsize=(27, 10))  # Adjust the figure size as necessary
    ...
    for pop_code_k3, group_data_k3 in admixture_k3.groupby('population_code'):
        ...
        tick_labels_k3.append(pop_code_k3)
    ...
    ax_k3.set_xticklabels(tick_labels_k3, rotation=45, ha='center')  # Adjusted alignment to 'center' for k=3

    # Set other labels and title for k=3
    ax_k3.set_xlabel('Population')
    ax_k3.set_ylabel('Ancestry Proportion')
    ax_k3.set_title(f'Admixture Results for Superpopulation: {superpopulation}')  # Updated title for k=3
    ...
    ax_k3.legend(handles=legend_patches_k3, bbox_to_anchor=(1.05, 1), loc='upper left')

    # Save the k3 plot
    plot_filename_k3 = f'admixture_{superpopulation}_k3.png'
    plot_path_k3 = os.path.join(images_dir, plot_filename_k3)
    plt.savefig(plot_path_k3)
    plt.close(fig_k3)

    # Create figure and axis for k=5
    fig_k5, ax_k5 = plt.subplots(figsize=(27, 10))  # Adjust the figure size as necessary
    ...
    for pop_code_k5, group_data_k5 in admixture_k5.groupby('population_code'):
        ...
        tick_labels_k5.append(pop_code_k5)
    ...
    ax_k5.set_xticklabels(tick_labels_k5, rotation=45, ha='center')  # Adjusted alignment to 'center' for k=5

    # Set other labels and title for k=5
    ax_k5.set_xlabel('Populations')
    ax_k5.set_ylabel('Ancestry Proportion')
    ax_k5.set_title(f'Admixture Results for Superpopulation: {superpopulation}')  # Updated title for k=5
...
def main_population_code(population_code):
    ...
    # Plot k=3 and k=5 results with smaller gaps and sample labels
    plot_admixture_with_sample_labels(admixture_k3, 3, colors_k3, population_code)
    plot_admixture_with_sample_labels(admixture_k5, 5, colors_k5, population_code)

    
    print('Admixture data retrieved and visualized successfully.')

# Main function to fetch and plot admixture data based on population_code or superpopulation
def main():
    population_or_superpopulation = "AFR"  # Replace with your desired code or superpopulation
